# Survival & Flourishing Fund (SFF) Funding Data

## Overview

This directory contains funding event data extracted from the Survival & Flourishing Fund.
...
1. Run dump helper:
   ```bash
   python scripts/extraction/new_dump.py --source sff --method web_scrape
   ```
...
5. Validate dump:
   ```bash
   python scripts/extraction/validate_dump.py --source sff --dump YYYY-MM-DD_HHMMSS
   ```
