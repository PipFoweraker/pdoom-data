...
"""Guard: does this PR actually qualify for the self-merge class it claims?
    ...
    python scripts/validation/check_self_merge_eligibility.py --base origin/main
...
control. It is also this repository's recurring failure mode, written down in
``.github/workflows/data-integrity.yml``: a check nothing runs is a check that
will be true until the day it is not.

R1's first condition is what this gate mechanises:
...
are not enough. This gate verifies that a run reference and a reason were
**stated**, not that the run exists or that it was red. Same trust boundary as
every declaration check here: it converts a silent omission into an attributable
statement. The reviewer clicks the link.

WHAT COUNTS AS DOCUMENTATION HERE
---------------------------------

  documentation = a path under ``docs/``, or a file ending ``.md`` / ``.rst`` /
  ``.txt``, EXCLUDING everything under ``data/`` and the machine-read files in
  ``NOT_DOCUMENTATION``.

``data/`` is excluded even though it holds many ``README.md`` files, and the
exclusion is the repo-specific part. The zones are the product: ``data/raw/`` is
immutable by contract, ``data/serveable/`` is a build output that must stay
byte-identical to a fresh projection, and a README inside a source directory is
that source's provenance record. A change under ``data/`` is a data change, to be
judged by the integrity gates, not waved through as prose. ``VERSION`` and the
``requirements*.txt`` pins are machine-read.
...
# Prose that is not documentation, for reasons specific to this repo.
NOT_DOCUMENTATION_PREFIXES = (
    "data/",  # the zones are the product; a README there is provenance, not prose
    "legacy/",  # frozen; corrections belong in the live docs, not the archive
)
...
NOT_DOCUMENTATION = (
    ...
    "requirements-checks.txt",  # dependency pin
)
...
def is_documentation(path):
    """True when the path alone proves the change is documentation."""
...
def _git(args, check=True):
    return subprocess.run(
        ...
        capture_output=True,
        text=True,
        # Explicit: text mode without this decodes git's output with the locale
        # codec (cp1252 on Pip's machine), so a non-ASCII path would crash the
        # gate rather than let it judge the PR.
        encoding="utf-8",
        errors="replace",
        check=check,
    ).stdout
...
def run(labels, paths, body):
    ...
    if docs and not guard:
        ...
        if not paths:
            ...
            findings.append(
                "%s is claimed, but this PR changes files that are not documentation.\n"
                "  The docs class is 'corrects documentation to match measured reality'\n"
                "  (R1) -- it is not a shortcut for a mixed PR.\n"
                "  Documentation here means: a path under docs/, or a file ending %s\n"
                "  -- excluding data/ (the zones are the product: raw/ is immutable by\n"
                "  contract, serveable/ is a build output, and a README inside a source\n"
                "  directory is that source's provenance record), legacy/, and the\n"
                "  machine-read files VERSION and requirements*.txt.\n"
                "  Not documentation:\n%s"
...
_RED_URL = "https://github.com/PipFoweraker/pdoom-data/actions/runs/1234567890"
...
SELF_TEST_CASES = (
    (
        ...
        [],
        ["scripts/build/project_taxonomy.py"],
        "",
        0,
    ...
    (
        ...
        ["class:docs"],
        ["docs/DATA_DICTIONARY.md", "README.md", "docs/adr/0001.md"],
        "",
        0,
        "MUST PASS: every path is documentation",
    ),
    (
        "docs class with a script smuggled in",
        ["class:docs"],
        ["docs/DATA_DICTIONARY.md", "scripts/build/project_candidates.py"],
        "",
        1,
        "MUST FAIL: the docs class is not a shortcut for a mixed PR",
    ),
    (
        "docs class touching a README inside a data zone",
        ["class:docs"],
        ["data/raw/alignment_research/README.md"],
        "",
        1,
        "MUST FAIL: .md suffix, but data/ is the product and raw/ is immutable",
    ),
    (
        "docs class touching VERSION",
        ["class:docs"],
        ["VERSION"],
        "",
        1,
        "MUST FAIL: the version SSOT is machine-read, not prose",
    ),
    (
